...
def books(A, B):
    ...
    while l <= h:
        ...
        if student_count(A, mid ) <= B:
            ...
            l = mid + 1  #increase pages

    return ans

def student_count(A, mid):
    count_sudents = 1
    count_pages = 0

    for i in range(len(A)):
        #add pages until they are less or equal to mid
        if count_pages + A[i] <= mid:
            count_pages +=A[i]
        else:
            #increase count and reset pages count to A[i]
            count_pages = A[i]
            count_sudents +=1

    return count_sudents
