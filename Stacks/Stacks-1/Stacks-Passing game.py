...
def passing_game(self, A, B, C):

    st = deque()
     
    #if postion of pass i.e A greater than the length of C, return as invalid
    if A > len(C):
        retuen "Invalid Pass"


    st.append(B) #append the intial person who holds the ball 

    
    for i in range(len(A)):
        #Keep popping if he pass is backwards, so the ball stays with the the one who is on the top of the stack
        if len(st) > 0 and C[i] == 0:
            st.pop()
        else:
            #append the value as ball is being passed
            st.append(C[i])

    return st[-1] #return the last ID who holds the ball
